...
void VulkanSampling::createColorResources(VkDevice device, VkPhysicalDevice physicalDevice, VkExtent2D swapChainExtent, VkFormat format) {
	...
	vulkanTM.createImage(device, physicalDevice, swapChainExtent.width, swapChainExtent.height, 1, colorFormat,
		VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, 1, msaaSamples,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 0, colorImage, colorImageMemory);

	colorImageView = vulkanTM.createImageView(device, colorImage, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1, 1, VK_IMAGE_VIEW_TYPE_2D);
}
